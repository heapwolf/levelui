...
var RelativeGrid = require("./relative-grid");
...
function textWidth(text) {
    return ansi.maxWidth((text || "").split("\n"));
}
...
function printLine(m, text, width) {
    m.grid.write(text + ansi.repeat(" ", width - ansi.charCount(text)) + m.pad());
}
...
function reflowLines(m, row, col, content) {
    ...
    var lines = ansi.fit(content, width).split("\n");
...
function Matrix(options) {
    this.grid = options.grid || new RelativeGrid(options.outputStream);
    this.columns = initColumns(options.columns || 1);
    this.padding = typeof options.padding === "number" ? options.padding : 1;
    this.rows = [];
    this.frozen = [];
    this.positions = [];
}
...
Matrix.prototype = {
    ...
    append: function (row, col, content) {
        ...
        if (pos.x - colPos + ansi.charCount(content) <= this.columns[col]) {
            this.grid.save();
    ...
    pad: function () {
        return ansi.repeat(" ", this.padding);
    },
